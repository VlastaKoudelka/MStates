...
MS_varmax = M * M_varimax_components;             %project original data into rotated space
...
varimax_loadings = diag(M*aprox_varimax_norm');
...
res_var_varimax = trace((M - aprox_varimax)*(M - aprox_varimax)')/(Nt*(Ns-1));
res_var_pca = trace((M - aprox_pca)*(M - aprox_pca)')/(Nt*(Ns-1));

%theoretical minimum residual variance
min_res_var_varimax = trace(M*M' - (aprox_varimax_norm*M').^2)/(Nt*(Ns-1));
